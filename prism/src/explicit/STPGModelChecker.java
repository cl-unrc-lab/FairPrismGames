...
public class STPGModelChecker extends ProbModelChecker
{
	...
	/**
	 * An upperbound just to try, it must be improved
	 */
	public static final double upperBound = 100;
	/**
	...
	/**
	 * Compute expected reachability rewards using value iteration from a Upper Bound
	 * this is the modifies version of iteration value defined in the paper.
	 * @param stpg The STPG
	 * @param rewards The rewards
	 * @param target Target states
	 ...
	 * Note: if 'known' is specified (i.e. is non-null, 'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult computeReachRewardsGFPValIter(STPG stpg, STPGRewards rewards, BitSet target, BitSet inf, boolean min1, boolean min2,
			double init[], BitSet known) throws PrismException
	{
		...
		if (init != null) {
			if (known != null) {
				...
				for (i = 0; i < n; i++)
					//soln[i] = soln2[i] = target.get(i) ? 0.0 : inf.get(i) ? Double.POSITIVE_INFINITY : init[i]; //esta linea esta probablemente mal
			...
			throws PrismException
	{
		...
		if (!allNonzero && !(rewards instanceof StateRewardsConstant)) {
			...
			if (verbosity >= 1) {
				mainLog.println("Changed: Computed an over-approximation of the solution (in " + timerApprox / 1000
			...
			BitSet known) throws PrismException
	{
		...
		timerApprox = System.currentTimeMillis();
		if (verbosity >= 1)
			mainLog.println("\n method computeReachRewardsZero: Starting expected reachability...");
		...
		if (!allNonzero && !(rewards instanceof StateRewardsConstant)) {
			...
			*/
			...
			//init = res.soln;
			//init = new double[n];
			// PC: Init with bigger number, Baier's approach need to be used here
		...
		timer = System.currentTimeMillis();
		//if (verbosity >= 1)
			mainLog.println("\nChanged :Starting expected reachability...");
	 ...
	 * Computes the reachability reward under the semantics where nonreaching
	 * runs get 0.
	 * PC: We modified this operator to provide the fairness behavior
	 * @param stpg
	 * @param rewards
	 * @param target
	 * @param min1
